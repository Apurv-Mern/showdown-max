...
function HostTeamsContent() {
  ...
  const sessionId =
    searchParams.get('sessionId') || (assignedSession?.id ? String(assignedSession.id) : '');
    ...
    <div className="max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <button
          ...
          className="inline-flex items-center gap-2 rounded-lg border border-neon-cyan/25 bg-neon-cyan/8 px-3 py-2 text-sm font-medium text-neon-cyan hover:bg-neon-cyan/14 transition-colors"
        >
          ...
          {teams.map((team, idx) => (
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  <span
                    className={`font-medium ${team.isEliminated ? 'line-through text-foreground/30' : ''}`}
                  >
                    {team.teamName}
                  </span>
      ...
      <Modal
        ...
        title={`Edit Score: ${showEditModal?.teamName}`}
      >
        <div className="space-y-4">
          <div>
